...
class RevenueController extends Controller
{
    ...
    public function RevenueByMonth()
    {
        ...
        $revenuesByYear = DB::table('invoices')
        ->select(DB::raw('YEAR(orderdate) as year'), DB::raw('SUM(total_amount) as total'))
        ->where('status',1)
        ...
        //dd($revenuesByWeek,$revenuesByMonth);
        $labels = [];
        $data = [];
        ...
        return view('backend.dashboard.index',compact('revenuedata','labels','template','revenueThisWeek','ordersThisWeek','revenueThisYear'));
    }
}
